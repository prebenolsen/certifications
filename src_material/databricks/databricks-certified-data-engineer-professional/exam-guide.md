...
Only the email and ltv columns will be returned; the email column will contain the
string "REDACTED" in each row.
Three columns will be returned, but one column will be named "REDACTED" and
contain only null values.
Only the email and ltv columns will be returned; the email column will contain all
null values.
...
Schedule a job to execute the pipeline once an hour on a dedicated interactive
...
Ingest the data, execute the narrow transformations, repartition to 2,048 partitions
